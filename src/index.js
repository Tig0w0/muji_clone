...
async function start() {
  const backendPromise = initializeData();

  if (supportsLocalLlm()) {
    updateBootstrapStatus('상품 데이터와 로컬 AI 모델을 함께 준비하고 있습니다.');
    loadLocalLlm(event => {
      if (typeof event?.progress !== 'number') return;
      const value = event.progress <= 1 ? event.progress * 100 : event.progress;
      updateBootstrapStatus(`상품 데이터와 로컬 AI 모델을 함께 준비 중입니다. AI ${Math.round(value)}%`);
    }).catch(error => {
