    ...
    <footer id="footer" data-chatbot="false" className="ScrollDown floating footer relative lg:border-t lg:border-neutral-200 false ">
      <div className="">
        <div className="Content mx-auto w-full max-w-[1440px] px-4 lg:px-[60px] 3xl:px-[100px] flex gap-6 overflow-x-scroll whitespace-nowrap py-4 text-[12px] font-semibold scrollbar-hide lg:py-5 lg:text-[14px]">
          <div className="flex items-center"><a href="#">About MUJI</a></div>
          <div className="flex items-center"><a href="#">채용공고</a></div>
          <div className="flex items-center"><a href="#">매장안내</a></div>
          <div className="flex items-center"><a href="#">이용약관</a></div>
          <div className="flex items-center font-extrabold"><a href="#">개인정보처리방침</a></div>
        </div>
      ...
      <div className="fixed bottom-[68px] right-4 z-[10000] lg:bottom-[40px] lg:right-5 lg:z-[11000] max-lg:bottom-[20px] max-lg:transition-[bottom] max-lg:duration-300">
        <div className="grid grid-cols-[38px_1fr] items-end lg:grid-cols-[40px_1fr]">
          <div className="flex flex-col gap-[10px] text-center transition-all duration-300 translate-y-0 max-lg:opacity-100 lg:gap-[10px]">
            {recentProduct && (
              <Link to={`/products/view/${recentProduct.product_id}`} className="inline-flex items-center justify-center font-medium gap-1 text-sm text-neutral-500 lg:hover:text-neutral-900 relative h-[38px] w-[38px] overflow-hidden rounded-full border border-neutral-100 bg-neutral-white transition-all duration-300 lg:h-10 lg:w-10 max-lg:translate-y-0 max-lg:opacity-0" title="최근 본 상품">
                <img alt={recentProduct.product_name || recentProduct.name_ko} loading="lazy" decoding="async" className="object-cover" src={recentImage} style={{position: 'absolute', height: '100%', width: '100%', inset: '0px', color: 'transparent'}} />
              </Link>
            )}
